    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if not torch.all(input_ids[:, -1] == 29871):
            ...
            special_characters = torch.unsqueeze(torch.Tensor([29871]).long(), dim=0)
            special_characters = special_characters.repeat(batch_size, 1)
            input_ids = torch.cat(
                (input_ids, special_characters.to(input_ids.device)), dim=1
            )
        ...
        predicted_action_token_ids = (
            ...
            generated_ids[:, -self.get_action_dim(unnorm_key) :]
            .cpu()
            .numpy()
        )
